...
class TestCreateMemoryRequest:
    ...
    def test_task_name_validation(self):
        ...
        for name in valid_names:
            request = CreateMemoryRequest(task_summary="测试总结", task_name=name)
        ...
        for name in invalid_names:
            with pytest.raises(ValidationError):
                CreateMemoryRequest(task_summary="测试总结", task_name=name)

    def test_task_summary_validation(self):
        """测试任务总结验证"""
        # 空字符串应该失败
        with pytest.raises(ValidationError):
            CreateMemoryRequest(task_summary="", task_name="test_task")
        ...
        # 有效的总结（会被trim）
        request = CreateMemoryRequest(
            task_summary="  有效的总结  ", task_name="test_task"
        )
...
这是任务总结"""
    ...
    async def test_create_cursor_memory_success(self, mcp_server, temp_dir):
        ...
        with patch("pathlib.Path.cwd", return_value=temp_dir):
            ...
            # 标准化路径分隔符进行比较
            expected_path = ".cursor/rules/user_login_implementation.mdc"
            actual_path = response_data["file_path"].replace("\\", "/")
...
成功完成了用户登录功能的实现"""
    ...
    async def test_create_cursor_memory_duplicate_filename(self, mcp_server, temp_dir):
        ...
        with patch("pathlib.Path.cwd", return_value=temp_dir):
            ...
            arguments = {"task_summary": "这是第二个任务", "task_name": "test_task"}
    ...
    async def test_create_cursor_memory_default_description(self, mcp_server, temp_dir):
        ...
        with patch("pathlib.Path.cwd", return_value=temp_dir):
            ...
            assert (
                'description: "get the summary of previous step: test_task"' in content
            )

    @pytest.mark.asyncio
    async def test_create_cursor_memory_permission_error(self, mcp_server):
        """测试权限错误处理"""
        # 模拟无法创建目录的情况
        with patch(
            ...
            with patch("pathlib.Path.cwd", return_value=Path("/fake/path")):
                arguments = {"task_summary": "任务总结", "task_name": "test_task"}
